...
function showImage(){
	...
	document.map_image.onerror = function(){remove_loading(this.varid,this.id); this.src='../images/noimage_daily.png';};
...
function changeInitHour(id){
	...
	//Forecast Hours
        ...
        var element = document.getElementById("forecasthour");
        for(i = element.options.length - 1 ; i >= 0 ; i--){element.remove(i);}
	if(selected_inithour=="init06z"){
		inithour_forecasthours = t06z_forecasthours;
        }
	else if(selected_inithour=="init12z"){
                inithour_forecasthours = t12z_forecasthours;
        }
	forecasthours = [];
        for(i=0; i<inithour_forecasthours.length; i++){
	if( (selected_inithour=="fhrmean") || (selected_inithour=="vertprof_fhrmean")){
        forecasthours.push({
                displayName:  "To F"+inithour_forecasthours[i],
                name: inithour_forecasthours[i],
                })
        }
        else{
        forecasthours.push({
                displayName: "F"+inithour_forecasthours[i],
                name: inithour_forecasthours[i],
                })
        }
	}
