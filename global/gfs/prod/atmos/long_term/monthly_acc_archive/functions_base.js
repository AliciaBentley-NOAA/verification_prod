...
function getURL(plottype,month,year,forecasthour,validhour,frame){
	var newurl = url.replace("PPP",plottype);
        var newurl = newurl.replace("MMM",month);
	var newurl = newurl.replace("YYY",year);
...
function showImage(){
	...
	var url = getURL(imageObj.plottype,imageObj.month,imageObj.year,imageObj.forecasthour,imageObj.validhour,i);
...
function changePlotType(id){
	...
	var selected_plottype = document.getElementById("plottype").value;
        ...
        forecasthours.push({
                ...
                name: "f"+plottype_forecasthours[i],
                })
        }
        else{
        forecasthours.push({
