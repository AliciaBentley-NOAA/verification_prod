...
function changePlotType(id){
	...
	for(i = element.options.length - 1 ; i >= 0 ; i--){element.remove(i);}
        if( (selected_plottype=="vertprof") || (selected_plottype=="vertprof_fhrmean")){
                plottype_vhrs = vertprof_vhrs;
                plottype_vhrs_name = vertprof_vhrs_name;
        }
        else{
                plottype_vhrs = non_vertprof_vhrs;
                plottype_vhrs_name = non_vertprof_vhrs_name;
        }
        validhours = [];
        for(i=0; i<plottype_vhrs.length; i++){
        validhours.push({
                displayName:  plottype_vhrs_name[i],
                name: plottype_vhrs[i],
                })
        }
	for(i=0; i<validhours.length; i++){
        var option = document.createElement("option");
