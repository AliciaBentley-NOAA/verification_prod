...
function getURL(metric,level,daterange,forecasthour,region,variable,frame){
    var newurl = url.replace("MMM",metric);
    ...
    if (region == "glb"){
	var newurl = newurl.replace("RRR","");
    }
    else {
	var newurl = newurl.replace("RRR","_"+region);
    }
    for(var i=0; i<5; i++){
...
function showImage(){
	...
	//document.getElementById('loading').style.display = "none";
        var url = getURL(imageObj.metric,imageObj.level,imageObj.daterange,imageObj.forecasthour,imageObj.region,imageObj.variable,i);
