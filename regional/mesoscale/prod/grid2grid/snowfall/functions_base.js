...
function populateMenu(mode){
	...
	else if(mode == 'derivedvar'){
                var element = document.getElementById("derivedvar");
                for(i = element.options.length - 1 ; i >= 0 ; i--){element.remove(i);}

                for(i=0; i<derivedvars.length; i++){
                        var option = document.createElement("option");
                        option.text = derivedvars[i].displayName;
                        option.value = derivedvars[i].name;
                        element.add(option);
                }
        }
	else if(mode == 'level'){
...
function getURL(plottype,metric,daterange,inithour,forecasthour,region,threshold,derivedvar,level,frame){
	...
	var newurl = newurl.replace("VVV",derivedvar);
...
function showImage(){
	...
	var url = getURL(imageObj.plottype,imageObj.metric,imageObj.daterange,imageObj.inithour,imageObj.forecasthour,imageObj.region,imageObj.threshold,imageObj.derivedvar,imageObj.level,i);
	...
	document.getElementById("derivedvar").selectedIndex = searchByName(imageObj.derivedvar,derivedvars);
...
function changeInitHour(id){
    ...
    var element = document.getElementById("forecasthour");
    for(i = element.options.length - 1 ; i >= 0 ; i--){element.remove(i);}
    if(selected_inithour=="00"||selected_inithour=="12"){
        if(selected_level=="06_a06"||selected_level=="a06"){
                init_forecasthours = accum06_forecasthours;
        }
	    else{
                init_forecasthours = z00_12_accum24_forecasthours;
	    }
    }
	else{
        init_forecasthours = z06_18_accum24_forecasthours;
	}
	forecasthours = [];
    for(i=0; i<init_forecasthours.length; i++){
        forecasthours.push({
            displayName:  init_forecasthours[i].toUpperCase(),
            name: init_forecasthours[i],
        })
    }
	for(i=0; i<forecasthours.length; i++){
        var option = document.createElement("option");
	...
	//Level
    var selected_level = document.getElementById("level").value;
    var selected_derivedvar = document.getElementById("derivedvar").value;
    var element = document.getElementById("level");
    for(i = element.options.length - 1 ; i >= 0 ; i--){element.remove(i);}
    if(selected_inithour=="00"||selected_inithour=="12"){
        if(selected_derivedvar=="asnow"){
            init_levels = z00_12_asnow_levels;
            init_levels_name = z00_12_asnow_levels_name;
        }        
        else if(selected_derivedvar=="weasd"){
            init_levels = z00_12_weasd_levels;
            init_levels_name = z00_12_weasd_levels_name;
        }
    }
	else{
        if(selected_derivedvar=="asnow"){
            init_levels = z06_18_asnow_levels;
            init_levels_name = z06_18_asnow_levels_name;
        }
        else if(selected_derivedvar=="weasd"){
            init_levels = z06_18_weasd_levels;
            init_levels_name = z06_18_weasd_levels_name;
        }
    }
	levels = [];
    for(i=0; i<init_levels.length; i++){
        levels.push({
            displayName:  init_levels_name[i],
            name: init_levels[i],
        })
    }
	for(i=0; i<levels.length; i++){
...
//Change the derived variable from dropdown menu
function changeDerivedVar(id){
        imageObj.derivedvar = id;
        preload(imageObj);
        showImage();
        document.getElementById("derivedvar").blur();
        var selected_derivedvar = document.getElementById("derivedvar").value;
	...
	//Level
        ...
        var element = document.getElementById("level");
        for(i = element.options.length - 1 ; i >= 0 ; i--){element.remove(i);}
        if(selected_derivedvar=="weasd"){
            if(selected_inithour=="00"||selected_inithour=="12"){
                derivedvar_levels = z00_12_weasd_levels;
		        derivedvar_levels_name = z00_12_weasd_levels_name;
            }
            else{
                derivedvar_levels = z06_18_weasd_levels;
		        derivedvar_levels_name = z06_18_weasd_levels_name;
            }
        }
        else{
            if(selected_inithour=="00"||selected_inithour=="12"){
                derivedvar_levels = z00_12_asnow_levels;
		        derivedvar_levels_name = z00_12_asnow_levels_name;
            }
            else{
                derivedvar_levels = z06_18_asnow_levels;
		        derivedvar_levels_name = z06_18_asnow_levels_name;
            }
        }
	levels = [];
        for(i=0; i<derivedvar_levels.length; i++){
        levels.push({
                displayName:  derivedvar_levels_name[i],
                name: derivedvar_levels[i],
                })
        }
	for(i=0; i<levels.length; i++){
        var option = document.createElement("option");
...
function changeLevel(id){
    ...
    var element = document.getElementById("forecasthour");
    for(i = element.options.length - 1 ; i >= 0 ; i--){element.remove(i);}
    if(selected_level=="a06_z0"||selected_level=="a06"){
        level_forecasthours = accum06_forecasthours;
    }
	else{
        if(selected_inithour=="00"||selected_inithour=="12"){
            level_forecasthours = z00_12_accum24_forecasthours;
        }
        else{
            level_forecasthours = z06_18_accum24_forecasthours;
        }
	}
	forecasthours = [];
