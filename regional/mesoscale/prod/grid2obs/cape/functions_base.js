...
function changePlotType(id){
	...
	for(i=element.options.length - 1 ; i >= 0 ; i--){element.remove(i);}
	if(selected_plottype=="timeseries"){
		plottype_forecasthours = timeseries_fhours;
		plottype_forecasthours_name = timeseries_fhours_name;
	}
	else {
		plottype_forecasthours = non_timeseries_fhours;
		plottype_forecasthours_name = non_timeseries_fhours_name;
	}
	forecasthours = [];
...
function changeMetric(id){
	...
	var element = document.getElementById("region");
	for(i = element.options.length - 1 ; i >= 0 ; i--){element.remove(i);}
	if(selected_metric=="fbias"||selected_metric=="csi"||selected_metric=="ctc"){
