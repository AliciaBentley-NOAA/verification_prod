...
function getURL(plottype,metric,daterange,inithour,forecasthour,region,threshold,frame){
...
function showImage(){
	...
	var url = getURL(imageObj.plottype,imageObj.metric,imageObj.daterange,imageObj.inithour,imageObj.forecasthour,imageObj.region,imageObj.threshold,i);
...
*/
    ...
    for(i=0; i<plottype_forecasthours.length; i++){
	    if(selected_plottype=="fhrmean"){
	        ...
	        forecasthours.push({
                displayName:  plottype_forecasthours[i],
                name: plottype_forecasthours[i],
            })
        }
        else{
            forecasthours.push({
                displayName: +plottype_forecasthours[i],
                name: "_"+plottype_forecasthours[i],
            })
        }
	}
	for(i=0; i<forecasthours.length; i++){
...
function changeInitHour(id){
        ...
        var selected_forecasthour = document.getElementById("forecasthour").value;
        var element = document.getElementById("forecasthour");
        og_forecasthours = threshmean_forecasthours;
        if(selected_inithour=="_init03z"||selected_inithour=="_init09z"||selected_inithour=="_init15z"||selected_inithour=="_init21z"){
                inithour_forecasthours = [];
                for(i=0; i<og_forecasthours.length; i++){
                        if(parseInt(og_forecasthours[i])+3 <= 384){
                            inithour_forecasthours[i]=NumToString(parseInt(og_forecasthours[i])+3);
                        }
                }
        }
        else {
                inithour_forecasthours = og_forecasthours;
        }
        if(selected_plottype=="threshmean"){
                for(i = element.options.length - 1 ; i >= 0 ; i--){element.remove(i);}
                forecasthours = [];
                for(i=0; i<inithour_forecasthours.length; i++){
                        forecasthours.push({
                            displayName:  inithour_forecasthours[i],
                            name: "_"+inithour_forecasthours[i],
                        })
                }
                for(i=0; i<forecasthours.length; i++){
                var option = document.createElement("option");
