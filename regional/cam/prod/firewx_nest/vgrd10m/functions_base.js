...
function populateMenu(mode){
	...
	else if(mode == 'obstype'){
                var element = document.getElementById("obstype");
                for(i = element.options.length - 1 ; i >= 0 ; i--){element.remove(i);}

                for(i=0; i<obstypes.length; i++){
                        var option = document.createElement("option");
                        option.text = obstypes[i].displayName;
                        option.value = obstypes[i].name;
                        element.add(option);
                }
        }
}
...
function getURL(plottype,metric,level,obstype,daterange,validhour,forecasthour,region,frame){
	...
	var newurl = newurl.replace("OOO",obstype);
...
function showImage(){
	...
	var url = getURL(imageObj.plottype,imageObj.metric,imageObj.level,imageObj.obstype,imageObj.daterange,imageObj.validhour,imageObj.forecasthour,imageObj.region,i);
	...
	document.getElementById("obstype").selectedIndex = searchByName(imageObj.obstype,obstypes);
...
function changePlotType(id){
	...
	else if(selected_plottype=="fhr"){
                plottype_forecasthours = fhr_forecasthours;
        }
	...
	if( (selected_plottype=="fhr") || (selected_plottype=="leaddate")){
        forecasthours.push({
        ...
        forecasthours.push({
                displayName: "Forecast hours 1-36",
                name: plottype_forecasthours[i],
                })
        }
	}
...
//Change the obstype from dropdown menu
function changeObstype(id){
        imageObj.obstype = id;
        preload(imageObj);
        showImage();
        document.getElementById("obstype").blur();
}
