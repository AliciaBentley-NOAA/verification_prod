...
function getURL(plottype,metric,daterange,inithour,forecasthour,level,region,frame){
...
function showImage(){
	...
	var url = getURL(imageObj.plottype,imageObj.metric,imageObj.daterange,imageObj.inithour,imageObj.forecasthour,imageObj.level,imageObj.region,i);
...
function changePlotType(id){
	...
	else if(selected_plottype=="vertprof"){
                plottype_forecasthours = vertprof_forecasthours;
        }
	else if(selected_plottype=="vertprof_fhrmean"){
                plottype_forecasthours = vertprof_fhrmean_forecasthours;
        }
	forecasthours = [];
        for(i=0; i<plottype_forecasthours.length; i++){
	if( (selected_plottype=="fhrmean") || (selected_plottype=="vertprof_fhrmean")){
        forecasthours.push({
	...
	//Levels
        ...
        var element = document.getElementById("level");
	for(i = element.options.length - 1 ; i >= 0 ; i--){element.remove(i);}
	if( (selected_plottype=="vertprof") || (selected_plottype=="vertprof_fhrmean")){
		plottype_levels = vertprof_levels;
		plottype_levels_name = vertprof_levels_name;
	}
        else{
		plottype_levels = non_vertprof_levels;
                plottype_levels_name = non_vertprof_levels_name;
        }
        levels = [];
        for(i=0; i<plottype_levels.length; i++){
        levels.push({
                displayName:  plottype_levels_name[i],
                name: plottype_levels[i],
                })
        }
	for(i=0; i<levels.length; i++){
        var option = document.createElement("option");
...
function changeRegion(id){
	...
	if(selected_region=="alaska"){
		region_inithours = alaska_inithours;
		region_inithours_name = alaska_inithours_name;
    }
	else{
        region_inithours = non_alaska_inithours;
		region_inithours_name = non_alaska_inithours_name;
    }
	inithours = [];
    for(i=0; i<region_inithours.length; i++){
        inithours.push({
            displayName: region_inithours_name[i],
            name: region_inithours[i],
        })
	}
	for(i=0; i<inithours.length; i++){
        var option = document.createElement("option");
    ...
    var element = document.getElementById("forecasthour");
    for(i = element.options.length - 1 ; i >= 0 ; i--){element.remove(i);}
	if(selected_region=="hawaii"){
		region_forecasthours = hawaii_forecasthours;
    }
	else{
        region_forecasthours = non_hawaii_forecasthours;
    }
	forecasthours = [];
    for(i=0; i<region_forecasthours.length; i++){
        forecasthours.push({
            displayName: "F"+region_forecasthours[i],
            name: region_forecasthours[i],
        })
	}
	for(i=0; i<forecasthours.length; i++){
        var option = document.createElement("option");
