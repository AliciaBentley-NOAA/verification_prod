...
function changeMetric(id){
	...
	//change levels (thresholds)
	var selected_level = document.getElementById("level").value;
        var element1 = document.getElementById("level");

        var selected_validhour = document.getElementById("validhour").value;
        var element2 = document.getElementById("validhour");


        for(i = element1.options.length - 1 ; i >= 0 ; i--){element1.remove(i);}
        for(i = element2.options.length - 1 ; i >= 0 ; i--){element2.remove(i);}

        if((selected_metric=="crps") || (selected_metric=="bias") || (selected_metric=="rmse_spread")) {
             metric_levels = crps_levels;
             metric_levels_name = crps_levels_name;
	     metric_validhours = crps_validhours;
             metric_validhours_name = crps_validhours_name;

        }
	else if((selected_metric=="ets") || (selected_metric=="fbias")) {
             metric_levels = ets_levels;
             metric_levels_name = ets_levels_name;
             metric_validhours = ets_validhours;
             metric_validhours_name = ets_validhours_name;
        }
	...
	validhours = [ ];
        ...
        for(i=0; i<metric_validhours.length; i++){
        validhours.push({
                displayName: metric_validhours_name[i],
                name: metric_validhours[i],
                })
        }
        ...
        option1.text = levels[i].displayName;
        option1.value = levels[i].name;
        element1.add(option1);
        }
	for(i=0; i<validhours.length; i++){
        var option2 = document.createElement("option");
        option2.text = validhours[i].displayName;
        option2.value = validhours[i].name;
        element2.add(option2);
        }

	var levels_values= [];
        for(i=0; i<element1.options.length; i++){
	       //window.alert(element1.options[i].value);
               levels_values.push(element1.options[i].value);
        }
	var validhours_values= [];
        for(i=0; i<element2.options.length; i++){
	       //window.alert(element2.options[i].value);
               validhours_values.push(element2.options[i].value);
        }

        if(levels_values.indexOf(selected_level) != -1){
               var idx = levels_values.indexOf(selected_level);
		window.alert("level");
               element1.options[idx].selected = true;
               //element1.onchange();
        }
        else{
